...
A matriz vive em código, não em prosa — `CHANNEL_CAPABILITIES` em `lib/channels/capabilities.ts`,
com uma linha por provider e uma coluna por capability, testada exaustivamente em
`tests/unit/channel-capability-matrix.test.ts`. Hoje tem três providers:

| Provider | `freeformOutsideWindow` | `requiresTemplates` | `banRisk` | `minIntervalMs` | `voiceNote` | `groups` | `costPerMessage` |
|---|---|---|---|---|---|---|---|
| `waha` | `true` | `false` | `true` | `null` | `server-convert` | `full` | `false` |
| `meta_cloud` | `false` | `true` | `false` | `6000` | `opus-only` | `limited` | `true` |
| `evolution` | `true` | `false` | `true` | `null` | `server-convert` | `full` | `false` |

`evolution` herda a física do `waha` (mesma engine Baileys) — ver `lib/channels/capabilities.ts`.[^evolution]

[^evolution]: Não é coincidência: WAHA/NOWEB e Evolution API são dois wrappers HTTP
    diferentes em cima do mesmo motor não-oficial (Baileys). Auto-restrição, banRisk e
    ausência de janela de 24h valem igual nos dois — o que muda entre eles é só a forma
    de autenticar e o transporte (`lib/waha/` vs. `lib/evolution/`), nunca a física do
    canal.
