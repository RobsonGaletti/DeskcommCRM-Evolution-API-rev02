...
| J2.8 | Conectar via **Evolution API** — sem provider nenhum configurado (`EVOLUTION_API_BASE_URL`/`WAHA_API_BASE_URL` vazios, estado real de 1ª instalação) | tela não quebra; mostra estado/banner de "nenhum provider configurado" — **PARCIAL, ver nota abaixo** |
| J2.9 | Conectar via Evolution API: escolher o provider no seletor, escanear QR com número de teste real, mandar mensagem de fora e ver chegar no inbox | **PENDENTE — não executado.** Precisa da stack `evolution-api`+`evolution-postgres`+`evolution-redis` (docker-compose, Task 8) no ar e um número de WhatsApp de teste para escanear o QR; nenhuma sessão desta série de tasks teve os dois disponíveis. Ver "AÇÃO PENDENTE" no relatório da Task 9. |

**Limitação conhecida v1 (revisão final, achado Important):** ao contrário do WAHA, o pipeline de
ingestão da Evolution (`lib/evolution/ingest.ts`, `handleInbound`) descarta mensagens com
`data.key.fromMe === true` — quando o operador manda uma mensagem direto do celular (fora do
composer do CRM), ela NÃO aparece na timeline. Decisão de escopo v1 aceitável, mas que só J2.9
(quando executado com instância real) consegue exercitar de fato; até lá fica registrada aqui
como gap conhecido, não como bug a investigar.

**Nota honesta sobre J2.8 (Task 9, 2026-09-10):** a ferramenta de browser (`claude-in-chrome`)
não conectou nesta sessão (`tabs_context_mcp` retornou "Browser extension is not connected"
mesmo após retry) — **nenhum screenshot real foi capturado**, e nenhuma tentativa de simular
um foi feita. O que FOI verificado, com o app rodando via `next build && next start`
(produção, porta 3900) contra um Postgres/Auth locais frescos (`baseline.sql` + GoTrue via
`supabase start`, owner criado por `bootstrap-owner.ts`, sem `onboarded_at`):
- `curl` (diagnóstico, não prova de UX) confirma que o processo sobe e responde: `GET /login`
  → 200; `GET /api/v1/health` → 503 com `{"redis":"down","waha":"down"}` coerente (nada real
  no ar) e `{"supabase":"ok"}`.
- Leitura do código-fonte (`app/onboarding/connect-whatsapp/_client.tsx:150-158` e
  `app/onboarding/connect-whatsapp/page.tsx`) confirma que, com `WAHA_API_KEY` no valor
  sentinela `dev_plaintext_change_me` (ou vazio) e `EVOLUTION_API_BASE_URL`/`EVOLUTION_API_KEY`
  vazios, `wahaConfigured` e `evolutionConfigured` ficam `false`, `configuredAtLeastOne` fica
  `false`, e a tela renderiza o banner `"Nenhum serviço de WhatsApp está configurado."` com o
  texto exato citado no JSX — a tela não lança exceção nesse caminho (nenhum `.map`/acesso a
  campo sem guarda antes do branch do banner). O mesmo padrão (`evolutionConfigured &&`) protege
  `components/connections/ConnectionsClient.tsx:161-194`.
- **Isto é inferência por leitura de código + diagnóstico de processo, não prova visual.**
  J2.8 fica `PARCIAL`, não `PASS`: falta repetir isto com o browser realmente conectado antes
  de considerar a tela provada.
